...
public class TeleportManager {
	...
	public void registerTeleport(final Player destinator, final boolean to, final Player executer) {
		if (exec.containsKey(executer.getName())) {
			...
			int xpneeded = (int) (plugin.getEntfernung(executer.getLocation(), destinator.getLocation()) * plugin.getConfig()
					.getDouble("teleport.xpperblock"));
			...
			if (executer.getTotalExperience() >= xpneeded) {
				...
				// plugin.PlayerLogger(player2,
				// String.format(plugin.getConfig().getString("teleport.teleportrequesttoplayer."
				// + plugin.config.language), player2.getName(),
				// plugin.getConfig().getInt("Cooldownoftp")), "");
				// plugin.PlayerLogger(executer,
				// String.format(plugin.getConfig().getString("teleport.teleportmustbeaccepted."
				// + plugin.config.language), player2.getName(),
				// plugin.getConfig().getInt("Cooldownoftp")), "");
				// plugin.PlayerLogger(executer,
				// String.format(plugin.getConfig().getString("teleport.info." +
				// plugin.config.language), xpneeded, entfernung), "Warning");
				plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable() {
					...
					public void run() {
						if (requested.containsKey(destinator.getName())) {
							...
							plugin.PlayerLogger(destinator, String.format(plugin.getConfig().getString("teleport.teleportrequesttimeoutaccept."
									+ plugin.config.language), plugin.getConfig().getInt("Cooldownoftp")), "Warning");
							plugin.PlayerLogger(executer, String.format(plugin.getConfig().getString("teleport.teleportrequesttimeout."
									+ plugin.config.language), plugin.getConfig().getInt("Cooldownoftp")), "Warning");
						}
					}
				}, plugin.getConfig().getInt("Cooldownoftp") * 20);
			} else {
				plugin.PlayerLogger(executer, String
						.format(plugin.getConfig().getString("teleport.notenoughxp." + plugin.config.language), entfernung, xpneeded), "Error");
			}
		}
	}
	...
	public void Teleportto(Player accepter) {
		...
		if (teleporter.getTotalExperience() >= xpneeded) {
			...
			teleporter.saveData();
			plugin.PlayerLogger(teleporter, String
					.format(plugin.getConfig().getString("teleport.teleportrequest." + plugin.config.language), teleporter.getName()), "");
			plugin.PlayerLogger(accepter, String
					.format(plugin.getConfig().getString("teleport.teleportrequest." + plugin.config.language), teleporter.getName()), "");
		} else {
			plugin.PlayerLogger(teleporter, String
					.format(plugin.getConfig().getString("teleport.notenoughxp." + plugin.config.language), entfernung, xpneeded), "Error");
	...
	public void Teleporttoexecutor(Player accepter) {
		...
		if (teleporter.getTotalExperience() >= xpneeded) {
			...
			accepter.saveData();
			plugin.PlayerLogger(teleporter, String
					.format(plugin.getConfig().getString("teleport.teleportrequest." + plugin.config.language), accepter.getName()), "");
			plugin.PlayerLogger(accepter, String.format(plugin.getConfig().getString("teleport.teleportrequest." + plugin.config.language), accepter
					.getName()), "");
		} else {
			plugin.PlayerLogger(teleporter, String
					.format(plugin.getConfig().getString("teleport.notenoughxp." + plugin.config.language), entfernung, xpneeded), "Error");
